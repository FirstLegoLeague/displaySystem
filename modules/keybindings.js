displaySystem.registerModule({
    ...
    factory: function(config) {
        ...
        // From David Flanagan's Definitive Guide
        // https://github.com/luics/Luy.Web/blob/master/!reading-note/!javascript-the-definitive-guide/17/Keymap.js
        var Keymap = (function() {
            /*
        ...
        var map = new Keymap(normalizeMap(config));

        //converts ctrl-w to ctrl_w for keymap class
        //converts string handlers to functions
        function normalizeMap(map) {
            return Object.keys(map).reduce(function(norm,key) {
                //convert dash to underscore
                var normkey = key.replace(/-/g,'_');
                var handler = (typeof map[key] === 'function')?map[key]:createHandler(map[key]);
                norm[normkey] = handler;
                return norm;
            },{});
        }

        //create a function that executes in the displaysystem modules
        function createHandler(str) {
            return new Function('displaySystem.modules.'+str);
        }
